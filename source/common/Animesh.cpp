...
void Animesh::MeshInit()
{
	...
	blendb_zero.rot.Identity();
	mat_zero = MATRIX::Scaling(blendb_zero.scale) * MATRIX::Rotation(blendb_zero.rot) * MATRIX::Translation(blendb_zero.pos);
};
...
void Animesh::SetupBoneMatrices()
{
	...
	for(word i=1; i<head.n_bones; ++i)
	{
		...
		model_to_bone[i] = bone.mat.Inverse();
		if(bone.parent > 0)
...
{
	out = MATRIX::Scaling(scale) * MATRIX::Rotation(rot) * MATRIX::Translation(pos) * mul;
}
